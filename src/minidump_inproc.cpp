...
namespace internal {
		...
		ULONG64 maxFileSize) noexcept
	{
		...
		ULONG64 procThreadRangeCount = 0, procThreadBytes = 0;
		ULONG64 indirectRangeCount = 0, indirectBytes = 0;
		ULONG64 memoryInfoRangeCount = 0;

		ULONG32 moduleCount = 0, moduleNameBytes = 0, moduleCodeViewBytes = 0, threadCount = 0;
		...
		if (g_ApisInitializeStatus != INITIALIZED) {
			SetLastError(ERROR_NOT_READY);
			return FALSE;
		}

		// The init-once globals (API table, system info) live in a PAGE_READONLY page set by
		// ProtectInitGlobals.  This check catches the case where someone VirtualProtect'd the
		// page back to writable and stomped on it, or where an early-init timing window let a
		// writer slip in before the protect took effect.
		if (!IsProtectedGlobalsIntact()) {
		...
		// ThreadInfoListStream is driven ONLY by MiniDumpWithThreadInfo. MiniDumpWithProcessThreadData is
		// a different request: it does not add a ThreadInfoList stream (verified against dbghelp's
		// .dumpdebug output) -- it adds the PEB/TEB/process-parameters/environment memory to the
		// MemoryList, which is handled by the process/thread-data capture below.
		...
		// MiniDumpWithProcessThreadData: capture the PEB, every thread's TEB, the process parameters, the
		// environment block and the strings they reference into the selected MemoryList (see
		// CaptureProcessThreadDataRanges). Only meaningful for selected-memory dumps; full-memory dumps
		// already include all of this.
		BOOL includeProcThread = (requestedFlags & MiniDumpWithProcessThreadData) != 0;
		...
		ULONG32 exceptionRva = 0, threadContextsRva = 0;
        ...
        if (hasException) { ++streamCount; exceptionStreamSize = sizeof(MINIDUMP_EXCEPTION_STREAM) + sizeof(CONTEXT); }
		...
		const ULONG64 fixedSize =
			...
			unloadedModuleListStorageSize +
			threadListStreamSize +
			threadContextsSize64 +
			exceptionStreamSize +
			threadInfoListSize64 +
			memoryInfoListSize64 +
			threadNamesStorageSize;
		...
		if (writeFullMemory) {
			...
			// Priority 1: process/thread data (PEB, TEBs, process parameters, environment, referenced
			// strings). High value for post-mortem analysis (drives !peb/!teb, the command line, the
			// current directory and the environment in WinDbg), so it is admitted ahead of data segments
			// and indirect memory. All-or-nothing against the remaining budget; a counting failure drops
			// it. Captured before dataSegs so its order in the byte stream matches the descriptor pass.
			if (includeProcThread) {
				if (!CaptureProcessThreadDataRanges(includeProcThread, threadCount,
					&procThreadRangeCount, &procThreadBytes)) {
					includeProcThread = FALSE;
					procThreadRangeCount = 0;
					procThreadBytes = 0;
				}
				else {
					ULONG64 procThreadCost =
						procThreadRangeCount * sizeof(MINIDUMP_MEMORY_DESCRIPTOR) + procThreadBytes;
					if (used + procThreadCost > effectiveBudget) {
						includeProcThread = FALSE;
						procThreadRangeCount = 0;
						procThreadBytes = 0;
					}
			...
			// Priority 2: writable data segments (all-or-nothing against the remaining budget; a
			// counting failure simply drops them).
			if (includeDataSegs) {
				if (!CaptureDataSegRanges(includeDataSegs, &extraRangeCount, &extraBytes)) {
			...
			// Priority 3: caller user streams. Higher priority than indirectly-referenced memory; each
			...
			// Priority 4 (lowest): indirectly-referenced memory, capped by whatever budget is left.
			if (includeIndirectMemory) {
				...
				if (cap != 0) {
					CollectKnownSelectedMemoryRanges(threadCount, includeDataSegs, includeProcThread);
			...
			selectedMemoryRangeCount =
				stackRangeCount + procThreadRangeCount + extraRangeCount + indirectRangeCount;
		}
		...
		if (hasException) {
			exceptionRva = nextRva;
			nextRva += exceptionStreamSize;
			exceptionProbe.ThreadContext.Rva = exceptionRva + sizeof(MINIDUMP_EXCEPTION_STREAM);
			exceptionProbe.ThreadContext.DataSize = sizeof(CONTEXT);
		}
		// All thread CONTEXT records sit contiguously; ThreadList entries and the ExceptionStream both
		// reference into this single block. 
		...
		if (writeSelectedMemory) {
			...
			const ULONG64 selectedMemoryBytes = stackBytes + procThreadBytes + extraBytes + indirectBytes;
		...
		header.TimeDateStamp = static_cast<ULONG32>(time(nullptr));
		...
		if (hasException) {
			...
			// exceptionStreamSize = sizeof(MINIDUMP_EXCEPTION_STREAM) + sizeof(CONTEXT)
			// if set directories[streamIndex].Location.DataSize = exceptionStreamSize
			// windbg .ecxr show errors like this:
			// Minidump doesn't have an exception context
			// Unable to get exception context, HRESULT 0x80004002
			directories[streamIndex].Location.DataSize = sizeof(MINIDUMP_EXCEPTION_STREAM);
			++streamIndex;
		}
		...
		if (!WriteAll(hFile, &header, sizeof(MINIDUMP_HEADER))) return FALSE;
    ...
    INPROC_EMIT_STREAM_RESULT((seekRva), StreamResultFromBool((writeExpr)))
		...
		if (writeFullMemory) {
			...
			INPROC_EMIT_STREAM(memoryListRva, WriteSelectedMemoryDescriptors(hFile, threadCount, includeDataSegs, \
				stackRangeCount, procThreadRangeCount, extraRangeCount, indirectRangeCount, memoryBaseRva));
		}
		if (hasException) {
            INPROC_EMIT_STREAM_RESULT(exceptionRva, WriteExceptionStream(hFile, contextProbe, &exceptionProbe));
		}
		...
		if (writeFullMemory) {
			...
			INPROC_EMIT_STREAM(memoryBaseRva, WriteSelectedMemoryBytes(hFile, threadCount, includeDataSegs, \
				procThreadRangeCount, indirectRangeCount));
		}
	...
	ULONG64 MaxFileSize) noexcept
{
	...
	// Serialize concurrent crashes: only one thread writes a dump at a time.  The kernel mutex
	// handle is stored in the PAGE_READONLY g_Protected page, so it cannot be corrupted by a
	// wild write.  Zero-timeout WaitForSingleObject acts as a non-blocking try-lock: a second
	// crasher gets WAIT_TIMEOUT and backs off immediately.
	HANDLE dumpMtx = minidump_inproc::internal::g_Protected.DumpMutex;
	if (dumpMtx == nullptr || WaitForSingleObject(dumpMtx, 0) != WAIT_OBJECT_0) {
		SetLastError(dumpMtx == nullptr ? ERROR_NOT_READY : ERROR_BUSY);
		return FALSE;
	}
	...
	COMMENT_STRING_OPER_TYPE oper) noexcept
{
	...
	wchar_t wsection[kIdCap] = { 0 };
	wchar_t wkey[kIdCap] = { 0 };
	wchar_t wvalue[kValCap] = { 0 };
	...
	__try {
		...
		if (value != nullptr) {
			if (MultiByteToWideChar(CP_ACP, 0, value, -1, wvalue, kValCap) == 0) {
				...
				int truncated = minidump_inproc::internal::TruncateMultiByteString(
					value, minidump_inproc::internal::kCommentMaxValueChars);
				if (MultiByteToWideChar(CP_ACP, 0, value, truncated, wvalue, kValCap) == 0) {
					return FALSE;
				}
			}
			pv = wvalue;
